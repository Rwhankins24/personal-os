'use strict'
// personal-os — Process Contact Sweep
// Reads ~/personal-os/data/contact-sweep.json (written by Cowork skill)
...
//
// Usage: node ~/personal-os/scripts/process-contact-sweep.js
...
const SWEEP_FILE  = path.join(__dirname, '../data/contact-sweep.json')
...
async function main() {
  ...
  if (!fs.existsSync(SWEEP_FILE)) {
    ...
    console.error('Run the contact-sweep-skill.md in Cowork first.')
    process.exit(1)
  }
    ...
    try {
      ...
      if (match) {
        ...
        const newContact = {
          ...
          notes:             `Imported from contact sweep (${extracted.confidence} confidence, ${extracted.email_count || 1} emails)`,
        }
