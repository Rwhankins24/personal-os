...
async function main() {
  console.log('Loading open pending_decisions...')

  const { data: items, error } = await supabase
    .from('pending_decisions')
    .select('id, title, context, source_type, urgency, project_id, created_at, potential_duplicate_of')
    .eq('status', 'open')
  ...
  for (const [clusterName, clusterItems] of Object.entries(clusters)) {
    ...
    const w = await runDedupWindows({
      ...
      archived,
      stats,
      tableName: 'pending_decisions',
      promptContext: 'pending decisions — open questions that need to be resolved',
      buildListLine: (item, i) =>
        `${i + 1}. "${item.title}"${item.urgency ? ' | urgency: ' + item.urgency : ''}${item.context ? ' | ' + item.context.slice(0, 80) : ''}`,
    })
