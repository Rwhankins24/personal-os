#!/bin/bash
LOG_FILE="$HOME/personal-os/logs/otter-upload.log"
JSON_FILE="$HOME/personal-os/data/last-otter-report.json"
TODAY=$(date +%Y-%m-%d)

export $(grep -v '^#' "$HOME/personal-os/api/.env" | xargs)

echo "[$TODAY $(date +%H:%M:%S)] Otter upload starting..." >> "$LOG_FILE"
...
print(d.get('otter_pull_completed_at') or '')" 2>/dev/null)

  UPLOAD_DONE=$(echo "$STATUS" | python3 -c \
    "import json,sys
d=json.load(sys.stdin)
print(d.get('otter_processing_completed_at') or '')" 2>/dev/null)
...
")
    if [ "$REPORT_DATE" = "$TODAY" ]; then
      echo "[$TODAY $(date +%H:%M:%S)] Uploading Otter report..." >> "$LOG_FILE"

      RESPONSE=$(curl -s -X POST \
        "$SUPABASE_URL/storage/v1/object/daily-reports/otter-$TODAY.json" \
      ...
      if [ "$HTTP_STATUS" = "200" ] || [ "$HTTP_STATUS" = "201" ]; then
        ...
        curl -s -X POST \
          ...
          -H "Content-Type: application/json" \
          -H "x-trigger-secret: $TRIGGER_SECRET" \
          -d "{\"step\":\"otter_processing\",\"run_date\":\"$TODAY\"}" \
          >> "$LOG_FILE" 2>&1
        exit 0
      else
